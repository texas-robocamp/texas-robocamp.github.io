...
The officially supported virtual machine software for this camp is **VirtualBox**. If you would prefer to use **VMWare** (an alternate software) instead, there are installation instructions for trial versions at the bottom of this page. Both will work for the purposes of this camp.
...
VMWare
Preinstall (Only for Windows)
Must enable virtualization in BIOS
Power on machine (or restart)
Press a function key repeatedly to go to BIOS as machine is starting (usually F10 or F12, can change based on your machine)
Go to configuration/advanced settings and enable virtualization (will say something like Virtualization/Virtual Technology/SVM)
Save changes and exit BIOS, then log back in


Downloading and Installing
Install Ubuntu 18.04 ISO and keep track of its download location (it is recommended to just download it to the Desktop for easy access)
Ubuntu 18.04 ISO 
Download and run the VMWare Installer (Fusion for Mac, Workstation Player for Windows/Linux) and walk through the installation prompts on the screen
Mac: Download VMware Fusion 11.5 (Click the link on the left) and double click the installer icon to start the installation
Windows/Linux: Download VMware Workstation Player and run the installer
Follow the instructions to install VMWare with defaults selected
Mac:
You will be prompted to enter in your password to allow Administrative Access while installing. Enter your password in.
IMPORTANT: The first time you install VMWare, you will see a pop-up saying something along the lines of “System software was blocked”. If you see this message:
Open System Preferences.
...
Setting up VM
Open up VMWare
Click on “Create a New Virtual Machine” (if the installation screen doesn’t come up automatically)
Find and add your downloaded Ubuntu ISO File (drag and drop for Mac)
Make sure easy install is checked
Set Username and Display Name: robocamp
Set Password: robocamp2020
Then, customize your settings
Mac: Finish creating your VM and the option to configure settings will automatically come up upon launch
...
Go to Hard Disk and set 15 GB Hard Drive Space (Mac)
Go to advanced settings and select “Pre allocate disk space” and unselect “Split into multiple files”, then click apply
Go to Memory (or Processors & Memory for Mac) and set 4GB (4096MB) memory, or about half of your available RAM
...
Go to Processors (or Processors & Memory for Mac) and set processor cores to 2 (or more if your computer has more available)
Check your processor if you are unsure -- a counselor can help you with this
Go to Display and uncheck “Accelerate 3D Graphics” (unless you have a discrete GPU)
Close settings and launch VM (it should launch automatically)
If prompted to Download/Install VMWare Tools, press “Download and Install”
Wait for installation to complete automatically, then log in and press next on all the prompts for the “Welcome to Ubuntu” window
If prompted to install Software Updates, just close the window (we will install them anyway in the process)
Note: When starting the VM, may get a message “Cannot connect virtual device because no corresponding device is available on the host. Press yes.

Installing Robocamp Packages
Note: When entering commands, it may ask for the password to “sudo”. This is just your user, so you can enter your normal password. The text will also be invisible as a security feature. 
Launch a new Terminal using Ctrl-Alt-T (Control-Option-T for Mac)
TODO: Launch website instructions, then copy paste commands from there
Enter the following commands (Tip: You can copy from and paste into the terminal using Ctrl-Shift-C and Ctrl-Shift-V)
sudo apt-key adv --keyserver keyserver.ubuntu.com --recv-keys C1CF6E31E6BADE8868B172B4F42ED6FBAB17C654
sudo add-apt-repository http://packages.ros.org/ros/ubuntu
sudo apt-key adv --keyserver keyserver.ubuntu.com --recv-keys D2486D2DD83DB69272AFE98867170598AF249743
sudo add-apt-repository http://packages.osrfoundation.org/gazebo/ubuntu-stable
...
Close the current terminal and open a new one using Ctrl-Alt-T.
...
At the bottom of the screen, you will see a bar with a bunch of numbers. Write down the number next to the FPS label - we may use this to check performance. (It will be changing; write down an approximate value.)
If your FPS is below 10, trying resizing your window down a little bit. This will help with performance!

Now open another terminal tab by right clicking on the terminal and clicking “open tab” . In this new terminal, type:
source /opt/ros/melodic/setup.bash
rosrun texas_robocamp teleop_texbot

This will let you manually drive the robot around. Make sure this works. The commands to drive the robot will show up in the terminal window.

Also make sure to resize your terminal windows so you can see both the robot and the terminal!

Note: Enter the commands to drive the robot in the terminal window, not in the window with the robot! 


If everything looks good, go back to the terminal and press Control+C in both terminal tabs to shut down the testing system and wait for all the windows to close completely. Once you see a “done” message, you can shut down the virtual machine:
Click the arrow in the top-right corner of the screen.
Click the power icon.
Click Power Off.
